...
validacion de filas, precios, techos, aprobado/rechazado, Drive y mail.
...
for _var in ("SMTP_HOST", "MAIL_TO", "GOOGLE_DRIVE_FOLDER_ID", "GOOGLE_DRIVE_CREDENTIALS_FILE",
             "GOOGLE_DRIVE_CREDENTIALS_JSON", "REQUIRE_DRIVE_UPLOAD", "FUENTES_HABILITADAS"):
...
def armar_excel(comunes=(), especiales=(), *, sub="1110111000", programa=None,
                hojas=("F7 común", "F7 especial"), encabezados=ENCABEZADOS):
...
class BaseCarga(unittest.TestCase):
    def setUp(self):
        ...
        # Los avisos por mail corren en el mismo hilo para poder verlos.
        parche = mock.patch.object(integrations, "en_segundo_plano", lambda f, *a, **k: f(*a, **k))
        parche.start()
        self.addCleanup(parche.stop)
...
class DriveYMail(BaseCarga):
    def setUp(self):
        super().setUp()
        _SmtpFalso.enviados = []
        for parche in (mock.patch.dict(os.environ, {"SMTP_HOST": "smtp.test", "MAIL_TO": "a@test, b@test"}),
                       mock.patch("smtplib.SMTP", _SmtpFalso)):
            parche.start()
            self.addCleanup(parche.stop)

    def adjuntos(self, mensaje):
        return {parte.get_filename(): parte.get_payload(decode=True) for parte in mensaje.iter_attachments()}

    def test_rechazo_manda_mail_con_el_excel_marcado(self):
        with mock.patch.object(integrations, "subir_a_drive") as subir:
            resp = self.subir(armar_excel(comunes=[comun(110, "ARROZ", None, G=0)]))
        self.assertEqual(resp.status_code, 422)
        subir.assert_not_called()  # a Drive va solo lo aprobado
        self.assertEqual(len(_SmtpFalso.enviados), 1)
        mensaje = _SmtpFalso.enviados[0]
        self.assertIn("RECHAZADO", mensaje["Subject"])
        self.assertEqual(mensaje["To"], "a@test, b@test")
        self.assertIn("F7 común, celda E11", mensaje.get_body().get_content())
        adjuntos = self.adjuntos(mensaje)
        self.assertEqual(list(adjuntos), ["F7_1110111000_22.01.00_con_errores.xlsx"])
        marcado = openpyxl.load_workbook(io.BytesIO(adjuntos["F7_1110111000_22.01.00_con_errores.xlsx"]))
        self.assertIn("Falta completar", marcado["F7 común"]["E11"].comment.text)
        self.assertIsNotNone(self.consultar("SELECT mail_enviado_en FROM f7_carga_excel")[0]["mail_enviado_en"])

    def test_techo_superado_queda_comentado_en_el_total_general(self):
        self.subir(armar_excel(comunes=[comun(110, "ARROZ", 101)]))
        adjunto = next(iter(self.adjuntos(_SmtpFalso.enviados[0]).values()))
        ws = openpyxl.load_workbook(io.BytesIO(adjunto))["F7 común"]
        total_general = next(c for (c,) in ws.iter_rows(min_col=1, max_col=1) if c.value == "Total general ")
        self.assertIn("techo presupuestario", ws[f"G{total_general.row}"].comment.text)

    def test_aprobado_va_a_drive_y_por_mail_con_el_original(self):
        contenido = armar_excel(comunes=[comun(110, "ARROZ", 1)])
        with mock.patch.object(integrations, "subir_a_drive",
                               return_value={"id": "abc", "webViewLink": "https://drive.test/abc"}) as subir:
            resp = self.subir(contenido)
            self.assertEqual(resp.status_code, 201, resp.get_json())
            subir.assert_called_once_with("F7_1110111000_22.01.00.xlsx", contenido, file_id_anterior=None)
            # Un archivo por Categoria: el segundo aprobado reemplaza al de Drive.
            self.subir(contenido)
            self.assertEqual(subir.call_args.kwargs["file_id_anterior"], "abc")
        mensaje = _SmtpFalso.enviados[0]
        self.assertEqual(mensaje["Subject"], "F7_1110111000_22.01.00 - aprobado - Salud")
        self.assertIn("https://drive.test/abc", mensaje.get_body().get_content())
        self.assertEqual(self.adjuntos(mensaje), {"F7_1110111000_22.01.00.xlsx": contenido})
    ...
    def test_si_drive_falla_no_se_aprueba(self):
        ...
        self.assertEqual(self.consultar("SELECT * FROM f7_submission"), [])
        self.assertEqual(_SmtpFalso.enviados, [])
...
class SubidaADrive(unittest.TestCase):
    """La logica de subir_a_drive con un cliente de Drive falso (la
    libreria de Google no hace falta para correr las pruebas)."""
    ...
    def setUp(self):
        ...
        class Archivos:
            def __init__(self, error_update=None):
                self.error_update = error_update

            def update(self, **kwargs):
                llamadas.append(("update", kwargs))
                return Pedido(self.error_update or {"id": kwargs["fileId"], "webViewLink": "https://d/1"})

            def create(self, **kwargs):
                llamadas.append(("create", kwargs))
                return Pedido({"id": "nuevo", "webViewLink": "https://d/nuevo"})

        self.HttpError, self.Archivos = HttpError, Archivos
        modulos = {
            "googleapiclient": types.ModuleType("googleapiclient"),
            "googleapiclient.errors": types.SimpleNamespace(HttpError=HttpError),
            "googleapiclient.http": types.SimpleNamespace(MediaIoBaseUpload=lambda fh, mimetype: ("media", mimetype)),
        }
        for parche in (mock.patch.dict(sys.modules, modulos),
                       mock.patch.dict(os.environ, {"GOOGLE_DRIVE_FOLDER_ID": "carpeta",
                                                    "GOOGLE_DRIVE_CREDENTIALS_JSON": "{}"})):
            parche.start()
            self.addCleanup(parche.stop)

    def servicio(self, error_update=None):
        archivos = self.Archivos(error_update)
        return types.SimpleNamespace(files=lambda: archivos)

    def test_crea_en_la_carpeta_compartida(self):
        with mock.patch.object(integrations, "_servicio_drive", return_value=self.servicio()):
            resultado = integrations.subir_a_drive("F7_x_01.01.00.xlsx", b"datos")
        self.assertEqual(resultado["id"], "nuevo")
        accion, kwargs = self.llamadas[0]
        self.assertEqual(accion, "create")
        self.assertEqual(kwargs["body"], {"name": "F7_x_01.01.00.xlsx", "parents": ["carpeta"]})
        self.assertTrue(kwargs["supportsAllDrives"])

    def test_reemplaza_el_anterior_y_si_ya_no_existe_crea_otro(self):
        with mock.patch.object(integrations, "_servicio_drive", return_value=self.servicio()):
            self.assertEqual(integrations.subir_a_drive("a.xlsx", b"1", file_id_anterior="viejo")["id"], "viejo")
        with mock.patch.object(integrations, "_servicio_drive", return_value=self.servicio(self.HttpError(404))):
            self.assertEqual(integrations.subir_a_drive("a.xlsx", b"1", file_id_anterior="borrado")["id"], "nuevo")
        self.assertEqual([accion for accion, _ in self.llamadas], ["update", "update", "create"])

    def test_otro_error_de_drive_no_aprueba(self):
        with mock.patch.object(integrations, "_servicio_drive", return_value=self.servicio(self.HttpError(403))):
            with self.assertRaises(integrations.IntegracionError):
                integrations.subir_a_drive("a.xlsx", b"1", file_id_anterior="x")
